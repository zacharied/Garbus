...
namespace Garbus.Game.Tests.Editor
{
    ...
    public partial class TestSceneComposeSelection : GarbusTestScene
    {
        ...
        [Test]
        public void TestInspectorScrollAndRealEasingDropdownInteraction()
        {
            waitForComposer();
            placeDiagonalSlider();
            selectSliderOnLine();
            ...
            AddAssert("node selected", () => sliderBlueprint().SelectedNodes.Count, () => Is.EqualTo(1));
            AddUntilStep("Side and Easing controls appear", () =>
                composer.ChildrenOfType<MultiValueEnumDropdown<HorizontalDirection>>().Any()
                && composer.ChildrenOfType<MultiValueEnumDropdown<Easing>>().Any());

            AddStep("bound inspector viewport", () =>
            {
                harness.RelativeSizeAxes = Axes.X;
                harness.Height = 400;

                var scroll = composer.ChildrenOfType<BasicScrollContainer>()
                                     .Single(s => s.Name == "Right toolbox scroll");
                ((ExpandingToolboxContainer)scroll.Child).Padding = new MarginPadding
                {
                    Top = 5,
                    Left = 5,
                    Right = 5,
            ...
            BasicScrollContainer? inspectorScroll() => composer.ChildrenOfType<BasicScrollContainer>()
                .SingleOrDefault(s => s.Name == "Right toolbox scroll");
            MultiValueEnumDropdown<Easing> easingDropdown() =>
                composer.ChildrenOfType<MultiValueEnumDropdown<Easing>>().Single();

            AddUntilStep("inspector has positive scroll extent", () => inspectorScroll()?.ScrollableExtent > 0);

            double initialScroll = 0;
            double capturedTime = 0;
            AddStep("stop editor clock and wheel inspector", () =>
            {
                editorClock.Stop();
                initialScroll = inspectorScroll()!.Current;
                capturedTime = editorClock.CurrentTime;
                input.MoveMouseTo(inspectorScroll()!.ScreenSpaceDrawQuad.Centre);
                input.ScrollVerticalBy(-1);
            });
            AddAssert("editor clock is stopped", () => !editorClock.IsRunning);
            AddUntilStep("inspector wheel advances visible viewport", () => inspectorScroll()!.Current > initialScroll);
            AddAssert("inspector wheel does not reach editor timeline", () => editorClock.CurrentTime,
                () => Is.EqualTo(capturedTime).Within(0.01));

            AddRepeatStep("wheel Easing into viewport", () =>
            {
                input.MoveMouseTo(inspectorScroll()!.ScreenSpaceDrawQuad.Centre);
                input.ScrollVerticalBy(-1);
            }, 2);
            AddUntilStep("Easing header is visible", () =>
            {
                var headerBounds = easingDropdown().ChildrenOfType<DropdownHeader>().Single().ScreenSpaceDrawQuad.AABBFloat;
                var viewportBounds = inspectorScroll()!.ScreenSpaceDrawQuad.AABBFloat;
                return Math.Abs(inspectorScroll()!.Current - inspectorScroll()!.Target) < 0.01
                       && headerBounds.Top >= viewportBounds.Top
                       && headerBounds.Bottom <= viewportBounds.Bottom;
            });
            AddStep("open real Easing dropdown", () =>
            {
                var header = easingDropdown().ChildrenOfType<DropdownHeader>().Single();
                input.MoveMouseTo(header.ScreenSpaceDrawQuad.Centre);
                input.Click(MouseButton.Left);
            });
            AddUntilStep("Easing menu opened", () =>
                easingDropdown().ChildrenOfType<Menu>().Single().State == MenuState.Open);
            AddAssert("Easing menu has finite maximum height", () =>
                !float.IsPositiveInfinity(easingDropdown().ChildrenOfType<Menu>().Single().MaxHeight));

            Menu.DrawableMenuItem easingItem = null!;
            Easing selectedEasing = default;
            BasicScrollContainer menuScroll = null!;
            AddStep("capture below-page easing item", () =>
            {
                menuScroll = easingDropdown().ChildrenOfType<Menu>().Single()
                                             .ChildrenOfType<BasicScrollContainer>().Single();
                easingItem = easingDropdown().ChildrenOfType<Menu.DrawableMenuItem>()
                    .Where(item => item.Item is DropdownMenuItem<MultiValueEnumDropdown<Easing>.Choice> typed
                                   && !typed.Value.IsMixed
                                   && typed.Value.Value != firstControlPoint().SweepEasing
                                   && item.ScreenSpaceDrawQuad.TopLeft.Y >= menuScroll.ScreenSpaceDrawQuad.BottomLeft.Y)
                    .OrderBy(item => item.ScreenSpaceDrawQuad.TopLeft.Y)
                    .First();
                selectedEasing = ((DropdownMenuItem<MultiValueEnumDropdown<Easing>.Choice>)easingItem.Item).Value.Value;
            });
            AddAssert("target easing starts below first menu page", () =>
                easingItem.ScreenSpaceDrawQuad.TopLeft.Y,
                () => Is.GreaterThanOrEqualTo(menuScroll.ScreenSpaceDrawQuad.BottomLeft.Y));
            AddRepeatStep("wheel real easing menu", () =>
            {
                var menuBounds = menuScroll.ScreenSpaceDrawQuad.AABBFloat;
                var inspectorBounds = inspectorScroll()!.ScreenSpaceDrawQuad.AABBFloat;
                float left = Math.Max(menuBounds.Left, inspectorBounds.Left);
                float right = Math.Min(menuBounds.Right, inspectorBounds.Right);
                float top = Math.Max(menuBounds.Top, inspectorBounds.Top);
                float bottom = Math.Min(menuBounds.Bottom, inspectorBounds.Bottom);
                input.MoveMouseTo(new Vector2((left + right) / 2, (top + bottom) / 2));
                input.ScrollVerticalBy(-1);
            }, 2);
            AddAssert("menu wheel sets a scroll target", () => menuScroll.Target, () => Is.GreaterThan(0));
            AddUntilStep("menu wheel reaches target easing", () =>
            {
                if (easingItem is not { IsPresent: true })
                    return false;

                var itemBounds = easingItem.ScreenSpaceDrawQuad.AABBFloat;
                var menuBounds = menuScroll.ScreenSpaceDrawQuad.AABBFloat;
                var inspectorBounds = inspectorScroll()!.ScreenSpaceDrawQuad.AABBFloat;
                return menuScroll.Current > 0
                       && Math.Abs(menuScroll.Current - menuScroll.Target) < 1
                       && itemBounds.Top >= menuBounds.Top
                       && itemBounds.Bottom <= menuBounds.Bottom
                       && itemBounds.Top >= inspectorBounds.Top
                       && itemBounds.Bottom <= inspectorBounds.Bottom;
            });
            AddStep("click real below-page easing item", () =>
            {
                input.MoveMouseTo(easingItem.ScreenSpaceDrawQuad.Centre);
                input.Click(MouseButton.Left);
            });
            AddUntilStep("Easing selection applied", () => firstControlPoint().SweepEasing == selectedEasing);
        }
