...
namespace Garbus.Game.Tests.Editor
{
    ...
    public class TestChartPreviewClock
    {
        ...
        public void TestStoppedClockHoldsAuthoritativeTime()
        {
            ...
            clock.Apply(new ChartPreviewTransport(1, 2000, false, 1, 100));
        ...
        {
            var clock = new ChartPreviewClock(timestampFrequency: 1000);
            clock.Apply(new ChartPreviewTransport(1, 1000, true, 1, 10_000));
        ...
        public void TestReceiveDelayDoesNotResetSenderTimestampAnchor()
        {
            ...
            var state = new ChartPreviewTransport(1, 1000, true, 1, 10_000);
        ...
        {
            var clock = new ChartPreviewClock(timestampFrequency: 1000);
            clock.Apply(new ChartPreviewTransport(1, 5000, true, 1, 100));
            clock.Apply(new ChartPreviewTransport(2, 2400, false, 1, 200));
        ...
        public void TestBackwardsSeekSnapsImmediately()
        {
            var clock = new ChartPreviewClock(timestampFrequency: 1000);
            clock.Apply(new ChartPreviewTransport(1, 5000, true, 1, 100));
            clock.Apply(new ChartPreviewTransport(2, 1200, true, 1, 200));

            Assert.That(clock.CurrentTimeAt(200), Is.EqualTo(1200));
        }

        [Test]
        public void TestStaleRevisionIsIgnored()
        {
            var clock = new ChartPreviewClock(timestampFrequency: 1000);
            clock.Apply(new ChartPreviewTransport(2, 2000, true, 1.5, 500));
            clock.Apply(new ChartPreviewTransport(1, 9000, false, 0, 1000));

            Assert.That(clock.CurrentTimeAt(1500), Is.EqualTo(3500).Within(0.001));
        }

        [Test]
        public void TestEqualRevisionDoesNotResetClockAnchor()
        {
            var clock = new ChartPreviewClock(timestampFrequency: 1000);
            clock.Apply(new ChartPreviewTransport(2, 2000, true, 1.5, 500));
            clock.Apply(new ChartPreviewTransport(2, 9000, false, 0, 1000));
