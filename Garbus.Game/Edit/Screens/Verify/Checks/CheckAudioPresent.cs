...
{
    /// <summary>
    /// Reports an issue when <see cref="Charts.ChartMetadata.AudioFile"/> is empty
    ...
    public class CheckAudioPresent : ICheck
    {
        ...
        {
            string audioFile = context.Chart.Metadata.AudioFile;
            ...
            string fullPath = Path.Combine(context.ChartFile.Directory, audioFile);

            if (!File.Exists(fullPath))
                yield return new Issue(null, $"Audio file '{audioFile}' not found in chart directory.", Name);
        }
    }
}
