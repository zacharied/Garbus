...
| Doc | Scope |
|---|---|
| `osu-framework.md` | Framework primer: DI/BDL, drawable lifetime + transforms, input hierarchy, clock plumbing, test scene basics. Cross-cutting traps stated generally (vertical fill-flow collapses relative-size children, ScrollContainer content anchoring, lambda subscription leaks, manual drawable disposal), cross-linked to the domain-specific instances. Notes the local reference clones of osu / osu-framework at `C:\Users\zachd\Code\BAC\LocalDependencies` (a bare "reference clones live here" pointer, no port story). |
...
- `rg "BigAssCircle|\bBAC\b|PLAN-port|ISSUES.md|Phase4-Issues" -i` over the repo comes back clean of
  agent-facing references (source-file attribution headers and the LocalDependencies path in
  osu-framework.md are the only sanctioned mentions of the BAC directory).
