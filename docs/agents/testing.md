...
- **Tuning scenes for new visual features.** When adding or reshaping a visual, add a scene under
  `Tuning/` (pattern: `TestSceneSliderGlowTuning`) or `Profiling/`
  (`TestSceneHeadOnlySliderStream`) that isolates the visual with live-adjustable parameters, so the
  look can be tuned and regressions caught.
...
- **Don't write ordering-dependent tests.** If a test depends on ordering, resolve the order
  dynamically rather than assuming it. (An enforced rule — see AGENTS.md Rules.)
