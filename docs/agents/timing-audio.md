...
`ComboBreakSound.cs` is the one feedback sample that is not a hitsound: a Miss that breaks a combo
**above** `MINIMUM_BROKEN_COMBO` (50) plays `Samples/Gameplay/combo-break.wav`. It wraps a
`HitSoundContainer` and assigns its `Samples` in the BDL, so the asset is resolved from the store at
load time and the miss itself costs no lookup. `PlayScreen` owns the instance and calls
`PlayIfComboBroken` from `onNewResult` with the pre-judgement combo, on the `BreaksCombo()` branch
before the counter resets; the drawable sits **outside** the gameplay-clock subtree because it is a
one-shot reaction to a judgement, not something scheduled against gameplay time. Rewinds revert
scoring through `onRevertResult` but never un-play a sample. Pinned by
`TestSceneComboBreakSound` (threshold, non-Miss judgements, and that the sample resolves).

Assets that are synthesized rather than recorded are generated by the `WavGen` one-off tool
(`dotnet run --project WavGen -- <outdir>`) — the metronome ticks and `combo-break.wav`. Its output is
deterministic, so regenerating reproduces the committed bytes.
